...
function StoreCredibility({ listInstallApp, isLoading, isError, onClickResync }) {
    var _a, _b, _c;
    ...
    useEffect(() => {
        if (listInstallApp === null)
            return;
        if (countPoint >= 100 || countPoint === displaySpeed().speed)
            return clearInterval(interval.current);
    }, [countPoint, listInstallApp]);
    useEffect(() => {
        if (isLoading === true && listInstallApp === null)
            setCountPoint(0);
